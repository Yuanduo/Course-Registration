...
public class ScDAO extends BaseHibernateDAO {
	...
	public void delete(Sc persistentInstance) {
		...
		try {Transaction tran=getSession().beginTransaction();
	...
	public Sc findById(db.ScId id) {
		log.debug("getting Sc instance with id: " + id);
		try {
			Sc instance = (Sc) getSession().get("db.Sc", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Sc instance) {
		...
		try {
			List results = getSession().createCriteria("db.Sc").add(
