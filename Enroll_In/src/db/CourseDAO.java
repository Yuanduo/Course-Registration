...
public class CourseDAO extends BaseHibernateDAO {
	...
	public Course findById(java.lang.Integer id) {
		...
		try {
			Course instance = (Course) getSession().get("db.Course", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Course instance) {
		...
		try {
			List results = getSession().createCriteria("db.Course").add(
	...
	public List findByCdescription(Object cdescription) {
		return findByProperty(CDESCRIPTION, cdescription);
	}
